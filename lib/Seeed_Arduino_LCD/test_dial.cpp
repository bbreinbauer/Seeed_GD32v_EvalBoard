...
void plotNeedle(TFT_eSPI &tft, int value, byte ms_delay)
{
  ...
  char buf[8]; snprintf(buf, 8, "%4.0f", value);
...
void testDial(TFT_eSPI &tft) {
  tft.init();
  tft.setRotation(1);
  tft.fillScreen(TFT_BLACK);

  analogMeter(tft); // Draw analogue meter

  // Draw 6 linear meters
  byte d = 40;
  plotLinear(tft, "A0", 0, 160);
  plotLinear(tft, "A1", 1 * d, 160);
  plotLinear(tft, "A2", 2 * d, 160);
  plotLinear(tft, "A3", 3 * d, 160);
  plotLinear(tft, "A4", 4 * d, 160);
  plotLinear(tft, "A5", 5 * d, 160);

  updateTime = millis(); // Next update time
  uint32_t startTime = millis();

  while(updateTime - startTime < 10000) {
    ...
    if (updateTime <= millis()) {
        ...
        plotPointer(tft); // It takes aout 3.5ms to plot each gauge for a 1 pixel move, 21ms for 6 gauges
        
        plotNeedle(tft, value[0], 0); // It takes between 2 and 12ms to replot the needle with zero delay
